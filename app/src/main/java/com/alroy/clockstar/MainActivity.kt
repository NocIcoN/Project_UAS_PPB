...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(tab_layout,view_pager){tab, position ->
            when (position) {
                0 -> tab.icon = getDrawable(R.drawable.ic_baseline_visibility_24)
                1 -> tab.icon = getDrawable(R.drawable.ic_baseline_notifications_active_24)
                2 -> tab.icon = getDrawable(R.drawable.ic_baseline_timer_24)
            }
        }.attach()
}
}
